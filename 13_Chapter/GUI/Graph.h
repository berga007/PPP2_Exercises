...
private:
    ...
    Color lcolor{int(fl_color())};          // color for lines and characters (with default)
    Line_style ls{0};                           
...
struct Box : Shape{
    ...
    Box(Point xy, int ww, int hh, float rad)
        :w{ww}, h{hh}, r{rad}
    {
        if(h<=0 || w<= 0) error("Bad rectangle: non-positive width or height");
        if (r > (w <= h? w: h)/2) error("Corner radius given is too large");
        add(xy);
    }
...
struct Lines : Shape {                 // related lines
    ...
    // initialise from list of points
...
struct Regular_hexagon : Shape{
    Regular_hexagon(Point xy, int s);
    Point center() const;
    void set_length(int s);
    void draw_lines() const;
private:
    // distance from center to each point in the hexagon
    int s;
};
...
struct Regular_polygon : Shape {
    Regular_polygon(Point xy, int nn, int s);
    Point center() const;
    void set_length(int s);
    void draw_lines() const;
    int n_sides() const { return n; }
private:
    int n;
    int s;
};
...
struct Textbox : Box{
    Textbox(Point xy, int ww, int hh, string& s) 
        : Box(xy,ww,hh), label{Point{xy.x+ww/8, xy.y+(6/4)*hh}, s} {}
...
struct Arc : Shape{
    Arc(Point p, int ww, int hh, double ang1, double ang2)
        :w{ww}, h{hh}, a1{ang1}, a2{ang2}
        {
            add(Point(p.x-ww, p.y-hh));
        }
    ...
    void set_angle1(double ang1)
    {
        // check pre-condition
        if(ang1 > a2) error("First arc angle must be lower than the 2nd");
        a1 = ang1;
    }

    double angle1() const { return a1; }

    void set_angle2(double ang2)
    {
        // check pre-condition
        if(ang2 < a1) error("Second arc angle must be higher than the 1st");
    }

    double angle2() const { return a2; }

    void set_angles(double ang1, double ang2)
    {
        if(ang1 > ang2) error("1st arc should be lower than the 2nd");
        a1 = ang1;
        a2 = ang2;
    }
